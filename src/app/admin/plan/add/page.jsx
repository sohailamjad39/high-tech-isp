...
// Wrapper component that includes the SessionProvider
function AddPlanPageContent() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      ...
      // Stop loading once we know the user is authenticated
      setIsLoading(false);
    }
  ...
  const createPlanMutation = useMutation({
    mutationFn: async (planData) => {
      const response = await fetch('/api/admin/plan/add', {
        method: 'POST',
        headers: {
    ...
    onError: (error) => {
      ...
      setSubmitError(error.message || 'Failed to create service plan. Please try again.');
    },
    onSettled: () => {
    ...
    return (
      <div className="flex mt-15">
        ...
        <div className="mx-auto px-2 sm:px-2 md:px-3 py-8 w-full max-w-auto">
          <div className="mb-8">
            <h1 className="font-semibold text-gray-900 text-2xl">Add New Service Plan</h1>
        ...
        <div className="mb-8">
          <div className="flex sm:flex-row flex-col sm:justify-between sm:items-center">
            <div>
              <h1 className="font-semibold text-gray-900 text-2xl">Add New Service Plan</h1>
              <p className="mt-1 text-gray-500 text-sm">
                Create a new internet service plan for customers
              </p>
            </div>
            <button
              type="button"
        ...
        {submitError && (
          <div className="bg-red-50 mb-6 p-4 rounded-md">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="font-medium text-red-800">Error creating plan</h3>
